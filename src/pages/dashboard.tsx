...
import { FULL_DATE_FORMAT } from 'const/date-format';
    ...
    import { useVacationCount, useEmployeeCount, useHrManagerCount } from "lib/roq";
  ...
  },
{
  ...
  return (
    <AppLayout>
      <Flex mb='7' alignItems={{ base: 'flex-start'}} justifyContent={{ base: 'center', md: 'space-between'}} direction={{ base: 'column-reverse', md: 'row'}}>
        <Flex direction='column'>
          <Text fontSize={{ md: '3xl', base: '2xl'}} as='b' color="base.content">
            Hello {session.user?.email} 👋
          </Text>
          <Text opacity={0.6} fontSize={{ md: 'md', base: 'sm' }} lineHeight={{ md: '24px', base: '20px'}} color="base.content">This is how your dashboard could look like</Text>
        </Flex>
        <Flex alignItems='center' gap={2}>
          <CalendarDashboardIcon />
          <Text fontSize='md' opacity={0.6} as='b' color="base.content">{format(new Date(), FULL_DATE_FORMAT)}</Text>
        </Flex>
      </Flex>
      <Box>
        <Flex direction={{ base: 'column', md: 'row'}} gap={6}>
          <Stack direction='column' spacing={8} flex={{ base: 1, md: 7}}>
            <Box>
              <Box mb={4}>
                <Text color='base.content' as='b' fontSize='xl' lineHeight={{ md: '24px', base: '28px'}}>Stats</Text>
              </Box>
                  ...
                  <Box border="1px" borderColor={"base.300"} borderRadius='10px' key={i} p={{ base: 3,  md: '18px'}}>
                    <Flex direction='column'>
                      ...
                      {stat.isLoading ? (
                        ...
                        <>
                          <Text fontWeight={600} fontSize='2xl'>{stat.result}</Text>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                <Text as='b' fontSize='xl'>Latest Hr Managers</Text>
                <NextLink
                  href={`hr-managers`}
                  passHref
                  legacyBehavior
          ...
          <Stack direction='column' spacing={6} flex={{ base: 1, md: 3}}>
            {!appConfig.customerRoles.includes(inflection.humanize(session?.user?.roles[0].replace(/-/g, ' '))) && (
              <Box>
                <Flex mb={4} justifyContent={'space-between'}>
                  <Text as='b' fontSize='xl'>All Users</Text>
                  <NextLink
                    href='/users'
                    passHref
                    legacyBehavior
                  ...
                  {allUsers?.data.map((user) => (
                    <Box border="1px" borderColor={"base.300"} borderRadius='lg' p='3' key={user.id}>
                      <Flex alignItems='center' gap={3}>
                        ...
                        <Flex direction='column'>
                          <Text fontSize='sm' fontWeight={600}>{[user.firstName, user.lastName].filter(Boolean).join(' ') || '-'}</Text>
                          <Text fontSize='xs' fontWeight={400}>{user.email}</Text>
                        </Flex>
                      </Flex>
                    </Box>
                  ))}
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                <Text as='b' fontSize='xl'>Current User</Text>
              </Flex>
              <Stack direction='column' spacing={3}>
                <Box border="1px" borderColor={"base.300"} borderRadius='lg' p='3'>
                  <Flex alignItems='center' gap={3}>
                    <Avatar w={8} h={8}/>
                    <Flex direction='column'>
                      <Text fontSize='sm' fontWeight={600}>{[session?.user?.firstName, session?.user?.lastName].filter(Boolean).join(' ') || '-'}</Text>
                      <Text fontSize='xs' fontWeight={400}>{session.user?.email}</Text>
                    </Flex>
                  </Flex>
                </Box>
              </Stack>
